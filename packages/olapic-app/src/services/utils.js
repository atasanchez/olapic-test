...
const getMediaFromNearbyCities = places =>
  new Promise((resolve, reject) => {
    Promise.all([getPhotos(places), getVideos(places)])
      .then(resPhotosAndVideos => {
        let data = resPhotosAndVideos
          .reduce((firstItem, secondItem) => firstItem.concat(secondItem))
          .filter(x => x !== undefined && x.data.totalHits > 0)
          .map(x => x.data.hits);

        if (data.length > 0) {
          data = data
            .reduce((firstHit, secondHit) => firstHit.concat(secondHit))
            .slice(0, 15);
        }
...
export { getCurrentISSPositionAndNearbyCities, getMediaFromNearbyCities };
